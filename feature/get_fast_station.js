...
async function get_fast_station(longitude, latitude) {
    const city = await get_city(longitude, latitude);
    const res = await get_station_db(city, "Fast");
    ...
    if(res.success) {
        ...
        for (var i=0; i<loop; i++) {
            ...
            for(var j=0; j<duration.length-1; j++) {
                ...
                res.result[start+j].distance = Math.round(distance[j+1]);
            }
        }
    }
    return res;
...
module.exports = get_fast_station;